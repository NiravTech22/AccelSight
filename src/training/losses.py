...
class AccelSightMultiTaskLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        if mask.any():
            m_flat = mask.view(-1)
            # BBox Loss
            p_bbox = preds["bbox"].permute(0, 2, 3, 1).reshape(-1, 4)[m_flat]
            t_bbox = targets["gt_bbox"].permute(0, 2, 3, 1).reshape(-1, 4)[m_flat]
            loss_bbox = self.bbox_loss(p_bbox, t_bbox)
            ...
            emb_dim = preds["embedding"].shape[1]
            obj_embeddings = preds["embedding"].permute(0, 2, 3, 1).reshape(-1, emb_dim)[m_flat]
